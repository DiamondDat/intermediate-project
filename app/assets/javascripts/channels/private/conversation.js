App.private_conversation = App.cable.subscriptions.create("Private::ConversationChannel", {
  ...
  received: function(data) {
    ...
    var messages_visilbe = ConvMessagesVisibility(conversation);
    ...
    if(conversation.length) {
      ...
      var messages_list = conversation.find('.message-list');
...
$(document).on('click', '.conversation-window, .private-conversation', function(e) {
  ...
  var lastest_message = $('.messages-list ul li:last .row div', this);
  if (lastest_message.hasClass('message-received') && lastest_message.hasClass('unseen')) {
    var conv_id = $(this).find('.panel').attr('data-pconversation-id');
    // if conv_id doesn't exists, it means that conversation is opened in messenger
    if (conv_id == null) {
      var conv_id = $(this).find('.messages-list').attr('data-pconversation-id');
    }
    // mark conversation as seen in conversations menu list
    lastest_message.removeClass('unseen');
